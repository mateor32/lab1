...
import { Users, ArrowLeftRight, TrendingUp, Wallet, Shield, Zap } from "lucide-react";

const stats = [
  { label: "Total Balance", value: "$0.00", icon: Wallet, trend: "Connect backend" },
  { label: "Customers", value: "—", icon: Users, trend: "No data yet" },
  { label: "Transactions", value: "—", icon: TrendingUp, trend: "No data yet" },
];
    ...
    <div className="space-y-10">
      ...
      <div className="relative overflow-hidden rounded-2xl gradient-hero p-8 sm:p-12 animate-fade-in">
        <div className="absolute inset-0 opacity-10" style={{
          backgroundImage: "radial-gradient(circle at 20% 50%, hsl(168 80% 50% / 0.3) 0%, transparent 50%), radial-gradient(circle at 80% 20%, hsl(190 80% 50% / 0.2) 0%, transparent 50%)"
        }} />
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 mb-4">
            <Zap className="h-3.5 w-3.5 text-primary" />
            <span className="text-xs font-medium text-primary">Banking Platform</span>
          </div>
          ...
          <p className="text-primary-foreground/60 max-w-md">
            Manage customers, track transactions, and transfer funds — all in one place.
          </p>
        </div>
      </div>
      ...
      <div className="animate-fade-in" style={{ animationDelay: "0.3s" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
          <Link to="/customers" className="glass-card-hover group p-6 flex items-center gap-5">
            ...
            <div>
              <p className="font-display font-semibold text-foreground text-lg">Customers</p>
              <p className="text-sm text-muted-foreground">View and manage accounts</p>
            </div>
          </Link>
          <Link to="/transfers" className="glass-card-hover group p-6 flex items-center gap-5">
            <div className="h-14 w-14 rounded-2xl gradient-primary flex items-center justify-center shadow-primary shrink-0 transition-transform duration-300 group-hover:scale-110">
              <ArrowLeftRight className="h-7 w-7 text-primary-foreground" />
            </div>
            <div>
              <p className="font-display font-semibold text-foreground text-lg">Transfers</p>
      ...
      {/* Status */}
      <div className="glass-card p-5 flex items-center gap-3 max-w-md animate-fade-in" style={{ animationDelay: "0.4s" }}>
        <Shield className="h-5 w-5 text-muted-foreground" />
        <div>
          <p className="text-sm font-medium text-foreground">Backend Status</p>
          <p className="text-xs text-muted-foreground">Waiting for Spring Boot at localhost:8080</p>
        </div>
